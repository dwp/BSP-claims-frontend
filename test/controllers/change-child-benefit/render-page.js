...
const {renderPage} = require('../../../src/pages/change-child-benefit/functions');
const template = require('../../../src/pages/change-child-benefit/template.marko');
...
test('should pass session data to template as values', async t => {
  ...
  req.session[req.params.claimId] = {
    changeChildBenefitNumber: {values: claimResponse.eligibilityCriteria}
  };
  await renderPage(req, res);
...
test('should pass errors from session to template', async t => {
  ...
  req.session[req.params.claimId] = {
    changeChildBenefitNumber: {values, errors}
  };
  req.session[1].changeChildBenefitNumber = {values, errors};
