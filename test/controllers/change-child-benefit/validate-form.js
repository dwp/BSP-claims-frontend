...
const {validateForm} = require('../../../src/pages/change-child-benefit/functions');
...
test('should add request body form values to session if data is invalid', t => {
  ...
  validateForm(req, res, next);
  t.deepEqual(req.session[1].changeChildBenefitNumber.values, req.body);
});
...
test('should not add unexpected request body form values to childBenefit session if data is invalid', t => {
  ...
  t.deepEqual(req.session[1].changeChildBenefitNumber.values, {...testData, childrenCHIBinCBOL: ''});
});
...
test('should trim request body form values before adding to childBenefit session if data is invalid', t => {
  ...
  t.deepEqual(req.session[1].changeChildBenefitNumber.values, expectedValues);
});
...
test('should add presence error to session object if childrenCHIBinCBOL is not true or false', t => {
  ...
  t.deepEqual(req.session[1].changeChildBenefitNumber.errors, errors);
});
...
test('should not add presence error to session object if childrenCHIBinCBOL is false and childrenCHIB is blank', t => {
  const {req, res} = t.context.setup();

  req.body = {childrenCHIBinCBOL: 'false', childrenCHIB: ''};
  validateForm(req, res, next);

  t.is(typeof req.session[1], 'object');
});
