...
const {validateForm} = require('../../../src/pages/change-dependent-children/functions');
...
test('should add request body form values to session if data is invalid', t => {
  ...
  req.body = {...testData, pregnantAtDateOfDeath: ''};

  validateForm(req, res, next);

  t.deepEqual(req.session[1].changeDependentChildren.values, req.body);
});
...
test('should not add unexpected request body form values to dependentChildren session if data is invalid', t => {
  ...
  t.deepEqual(req.session[req.params.claimId].changeDependentChildren.values, {...testData, pregnantAtDateOfDeath: ''});
});
...
test('should add presence error to session object if dependentChildren is something other than "yes" or "no"', t => {
  const {req, res} = t.context.setup();

  const values = {...testData, dependentChildren: ''};
  const errors = {dependentChildren: 'dependent-children:form.dependentChildren.errors.presence'};

  req.body = values;
  validateForm(req, res, next);

  t.deepEqual(req.session[req.params.claimId].changeDependentChildren, {values, errors});
});
