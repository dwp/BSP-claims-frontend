...
const {validateForm} = require('../../../src/pages/change-payment-details/functions');
...
test('should not add unexpected request body form values to changePayment session if data is invalid', t => {
  const req = new FakeRequest();
  const res = new FakeResponse(req);
  ...
  t.deepEqual(req.session[1].changePayment.values, {...testData, accountType: ''});
});
...
test('should add presence error that account type was not selected', t => {
  ...
  t.deepEqual(req.session[1].changePayment, {values, errors});
});

test('should add presense error if account name field is blank', t => {
  const req = new FakeRequest();
  const res = new FakeResponse(req);
