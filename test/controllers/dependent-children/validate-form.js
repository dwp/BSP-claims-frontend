...
const {validateForm} = require('../../../src/pages/dependent-children/functions');
...
test('should add request body form values to session if data is invalid', t => {
  ...
  t.deepEqual(req.session[req.params.claimId].dependentChildren.values, req.body);
});
...
test('should not add unexpected request body form values to dependentChildren session if data is invalid', t => {
  ...
  t.deepEqual(req.session[req.params.claimId].dependentChildren.values, {...testData, pregnantAtDateOfDeath: ''});
});
...
test('should add presence error to session object if dependentChildren is something other than "yes" or "no"', t => {
  ...
  t.deepEqual(req.session[req.params.claimId].dependentChildren, {values, errors});
});
