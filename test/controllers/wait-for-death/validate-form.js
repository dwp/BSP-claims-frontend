...
const {validateForm} = require('../../../src/pages/wait-for-death/functions');
...
test('should add presence error to session objects if wait is blank', t => {
  ...
  const expectedErrors = {wait: 'wait-for-death:form.wait.errors'};

  validateForm(req, res);

  t.deepEqual(req.session[req.params.claimId].waitForDeath, {errors: expectedErrors});
});

test('should add presence error to session objects if wait field is something other than yes or no', t => {
  const {req, res} = t.context.setup();
