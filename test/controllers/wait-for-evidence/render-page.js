...
const {renderPage} = require('../../../src/pages/wait-for-evidence/functions');
const template = require('../../../src/pages/wait-for-evidence/template.marko');
...
test('should pass claimId to template', t => {
  ...
  req.session[req.params.claimId].waitForEvidence = {errors: {wait: 'This is required'}};
  req.claim = claimResponse;
...
test('should set waitForEvidence session object to undefined', t => {
  const {req, res} = t.context.setup();

  req.session[req.params.claimId].waitForEvidence = {test: 'test'};
  req.claim = claimResponse;

  renderPage(req, res);
  t.is(req.session[req.params.claimId].waitForEvidence, undefined);
});
